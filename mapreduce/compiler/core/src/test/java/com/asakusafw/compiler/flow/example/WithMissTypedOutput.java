...
/**
 * A jobflow class which has output w/ invalid exporter description.
 */
@JobFlow(name = "testing")
public class WithMissTypedOutput extends FlowDescription {
     ...
     * @param out output
     */
    public WithMissTypedOutput(
            @Import(name = "hoge", description = MockHogeImporterDescription.class)
            In<MockHoge> in,
            @Export(name = "hoge", description = InvalidTypeExporterDescription.class)
            Out<MockHoge> out) {
        this.in = in;
        this.out = out;
    }
