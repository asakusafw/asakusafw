...
/**
 * A jobflow which has output w/ abstract exporter class.
 */
@JobFlow(name = "testing")
public class WithAbstractExportOutput extends FlowDescription {
     ...
     * @param out output
     */
    public WithAbstractExportOutput(
            @Import(name = "hoge", description = MockHogeImporterDescription.class)
            In<MockHoge> in,
            @Export(name = "hoge", description = AbstractExporterDescription.class)
            Out<MockHoge> out) {
        this.in = in;
        this.out = out;
    }
