...
/**
 * Jobflow with empty name for output.
 */
@JobFlow(name = "testing")
public class WithEmptyOutputName extends FlowDescription {
     ...
     * @param out output
     */
    public WithEmptyOutputName(
            @Import(name = "hoge", description = MockHogeImporterDescription.class)
            In<MockHoge> in,
            @Export(name = "", description = MockHogeExporterDescription.class)
            Out<MockHoge> out) {
        this.in = in;
        this.out = out;
    }
