...
then
    ...
    do
        if [ "$_TD_LIBJARS" = "" ]
        then
            _TD_LIBJARS="$ASAKUSA_HOME/core/lib/$f"
        else
            _TD_LIBJARS="$_TD_LIBJARS,$ASAKUSA_HOME/core/lib/$f"
        fi
    done
fi
...
then
    ...
    do
        ...
        then
            if [ "$_TD_LIBJARS" = "" ]
            then
                _TD_LIBJARS="$_OPT_LIBRARIES_PATH/$f"
            else
                _TD_LIBJARS="$_TD_LIBJARS,$_OPT_LIBRARIES_PATH/$f"
            fi
        fi
    done
fi
...
then
    ...
    do
        if [ "$_TD_LIBJARS" = "" ]
        then
            _TD_LIBJARS="$ASAKUSA_HOME/ext/lib/$f"
        else
            _TD_LIBJARS="$_TD_LIBJARS,$ASAKUSA_HOME/ext/lib/$f"
        fi
    done
fi
