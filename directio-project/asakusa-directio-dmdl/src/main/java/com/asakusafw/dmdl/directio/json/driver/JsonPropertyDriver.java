...
/**
 * Processes <code>&#64;directio.json.property</code> attributes.
 * @since 0.10.3
 * @see JsonFormatConstants
 */
public class JsonPropertyDriver extends PropertyAttributeDriver {
    ...
    public void process(DmdlSemantics environment, PropertyDeclaration declaration, AstAttribute attribute) {
        ...
        JsonPropertySettings settings = JsonPropertySettings.consume(environment, attribute, elements);
        environment.reportAll(AttributeUtil.reportInvalidElements(attribute, elements.values()));
        JsonPropertyTrait trait = new JsonPropertyTrait(attribute, name, settings);
        if (trait.verify(environment, declaration)) {
            JsonPropertyTrait.register(environment, declaration, attribute, trait);
        }
    }

    @Override
    public void verify(DmdlSemantics environment, PropertyDeclaration declaration, AstAttribute attribute) {
        JsonPropertySettings settings = JsonPropertyTrait.getSettings(declaration);
        ...
        if (value.isPresent()) {
            ...
            if (Arrays.stream(kinds)
                ...
                environment.report(new Diagnostic(
                        ...
                        Messages.getString("JsonPropertyDriver.diagnosticInvalidType"), //$NON-NLS-1$
                        getTargetName(),
