...
/**
 * Processes <code>&#64;directio.line.file_name</code> attributes.
<h2>'&#64;directio.line.file_name' attribute</h2>
The attributed declaration must have no attributes.
 * @since 0.7.5
 */
public class LineFileNameDriver  extends PropertyAttributeDriver {

    /**
     * The attribute name.
     */
    public static final String TARGET_NAME = "directio.line.file_name"; //$NON-NLS-1$
    ...
    public void process(DmdlSemantics environment, PropertyDeclaration declaration, AstAttribute attribute) {
        ...
        LineFieldTrait.checkFieldType(environment, declaration, attribute, BasicTypeKind.TEXT);
        if (LineFieldTrait.checkConflict(environment, declaration, attribute)) {
            declaration.putTrait(
                    LineFieldTrait.class,
                    new LineFieldTrait(attribute, LineFieldTrait.Kind.FILE_NAME));
        }
    }
}
