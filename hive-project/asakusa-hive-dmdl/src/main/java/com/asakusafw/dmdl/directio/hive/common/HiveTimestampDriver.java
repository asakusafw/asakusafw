...
/**
 * Processes <code>&#64;directio.hive.timestamp</code> attributes.
<h2>'&#64;directio.hive.timestamp' attribute</h2>
The attributed declaration must be without any properties.
 * @since 0.7.0
 */
public class HiveTimestampDriver extends PropertyAttributeDriver {

    /**
     * The attribute name.
     */
    public static final String TARGET_NAME = "directio.hive.timestamp"; //$NON-NLS-1$
    ...
    public void process(DmdlSemantics environment, PropertyDeclaration declaration, AstAttribute attribute) {
        if (Util.checkProperty(environment, declaration, attribute, HiveFieldTrait.TypeKind.TIMESTAMP) == false) {
            return;
        }
