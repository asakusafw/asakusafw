...
* http://www.asakusafw.com/download/framework-organizer/asakusafw-organizer-0.7.6.tar.gz
...
以下は、 ``asakusafw.version`` に Asakusa Frameworkのバージョン ``0.7.6`` を指定する例です。
...
..  code-block:: xml
	...
	<properties>
		<asakusafw.version>0.7.6</asakusafw.version>
...
以下は、Asakusa Framework のバージョン ``0.7.6`` で提供している、Hadoop2系で利用するためのAsakusa Frameworkバージョンを指定する例です [#]_ 。

..  code-block:: sh
     
    cd asakusafw-organizer
    mvn package -Dasakusafw.version=0.7.6-hadoop2
...
..  code-block:: sh
    ...
    cd asakusafw-organizer
    mvn antrun:run -Dasakusafw.version=0.7.6-hadoop2
...
..  code-block:: sh
    ...
    cd asakusafw-organizer
    mvn package antrun:run -Dasakusafw.version=0.7.6-hadoop2
