...
以下は、 ``asakusafw.version`` に Asakusa Frameworkのバージョン ``0.5.0`` を指定する例です。
...
以下は、Asakusa Framework のバージョン ``0.5.0`` で試験的に提供している、CDH4で利用するためのAsakusa Frameworkバージョンを指定する例です。
...
..  code-block:: sh
    ...
    cd asakusafw-organizer
    mvn antrun:run -Dasakusafw.version=0.5.0-hadoop2
...
..  code-block:: sh
    ...
    cd asakusafw-organizer
    mvn package antrun:run -Dasakusafw.version=0.5.0-hadoop2
