...
YAESSの導入方法については :doc:`../application/administrator-guide` のYAESSに関する項目を参照してください。
...
アプリケーションの配備方法は、 :doc:`../application/administrator-guide` の「開発環境で作成したバッチアプリケーションのデプロイと動作確認」を参照してください。
...
また、 `バッチ全体の実行`_ と同様に、 ``-A <変数名>=<値>`` という形式で引数を指定できます
