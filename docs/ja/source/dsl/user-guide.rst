...
という名前のシステムプロパティに最適化オプションを指定できます [#]_ 。
このプロパティの値は、 ``+<項目名>`` を指定すると最適化の項目を有効にし、
``-<項目名>`` を指定すると無効にします。
...
..  list-table:: 最適化オプションの項目
    ...
    * - ``enableCombiner``
      ...
      - 部分集約 [#]_ の既定値。
        ...
        ``PartialAggregation.DEFAULT`` が [#]_ 指定された場合に、
