...
以降では、このサンプルアプリケーションをビルド、実行する *前に* 、Direct I/Oの設定方法を紹介します。
...
DSLで論理パスより長いパスを指定した場合、論理パスにマッチした残りの部分は、そのままファイルシステム上のパスに利用します。
...
..  warning::
    (再掲) ファイルシステムパス以下はテスト実行時に全て削除されます。
    特にスタンドアロンモードのHadoopを利用時に相対パスを指定した場合、
    ホームディレクトリを起点としたパスと解釈されるため注意が必要です。
...
#. ``<パッケージ名>.csv.Abstract<データモデル名>InputDescription``
#. ``<パッケージ名>.csv.Abstract<データモデル名>OutputDescription``
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
開発環境のCSVファイルをインポートしてHadoopの処理を行う場合、 `CSV入出力への対応`_ で生成した ``<パッケージ名>.csv.Abstract<データモデル名>InputDescription`` クラスのサブクラスを作成して必要な情報を記述します。
...
``String getResourcePattern()``
    ...
    なお、ここにはスラッシュ ( ``/`` )も含めることができますが、
    論理パスとのマッピングは ``getBasePath()`` に指定したパスのみを利用します。
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ジョブフローの処理結果をCSVファイルにエクスポートする場合、 `CSV入出力への対応`_ で生成した ``<パッケージ名>.csv.Abstract<データモデル名>OutputDescription`` クラスのサブクラスを作成して必要な情報を記述します。
...
``String getResourcePattern()``
    ...
    パターンには ``{property_name}`` (プレースホルダ) などを利用できます。
