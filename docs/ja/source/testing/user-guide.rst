...
下記の例は、 ``YourOperator`` クラスに作成した演算子 ``operatorName`` の入力ポート ``inputName`` に入力される全てのデータについてトレースの設定を行います。
...
下記の例は、 ``YourOperator`` クラスに作成した演算子 ``operatorName`` の出力ポート ``outputName`` から出力する全てのデータについてトレースの設定を行います。
...
..  note::
    演算子の入出力ポート名と、演算子メソッドの引数名は必ずしも一致しません。
    Flow DSLで利用する :doc:`演算子ファクトリメソッド <../dsl/user-guide>` の引数名が入力ポート名に該当し、
    :doc:`演算子オブジェクト <../dsl/user-guide>` のフィールド名が出力ポート名に該当します。
