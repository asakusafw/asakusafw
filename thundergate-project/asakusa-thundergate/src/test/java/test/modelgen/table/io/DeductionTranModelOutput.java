...
/**
 * {@link DeductionTran}をTSVなどのレコード形式で出力する。
 */
@Generated("ModelOutputEmitter:0.0.1")@SuppressWarnings("deprecation") public final class DeductionTranModelOutput
        implements ModelOutput<DeductionTran> {
    /**
     * 内部で利用するエミッター。
     */
     ...
     */
    public DeductionTranModelOutput(RecordEmitter emitter) {
        if(emitter == null) {
            throw new IllegalArgumentException();
        }
        this.emitter = emitter;
    }
    @Override public void write(DeductionTran model) throws IOException {
        ...
        emitter.emit(model.getDeductionNoOption());
        emitter.emit(model.getSellerCodeOption());
        emitter.emit(model.getBuyerCodeOption());
        emitter.emit(model.getClosedDateOption());
        ...
        emitter.emit(model.getDetailCountOption());
        emitter.endRecord();
    }
