...
class Classpath {
    ...
    public Session session(ClassLoader base) {
        return new Session(FilterClassLoader.create(
                entries.stream()
                        .filter(Files::exists)
                        .flatMap(it -> {
                            try {
    ...
    private static final class FilterClassLoader extends URLClassLoader {
        ...
        static final String[] CLASS_FILTER_PREFIX = {
                "com.asakusafw.",
                "org.slf4j.",
        };

        FilterClassLoader(URL[] url, ClassLoader parent) {
            super(url, parent);
        }

        static URLClassLoader create(URL[] url, ClassLoader parent) {
            return AccessController.doPrivileged(
                new PrivilegedAction<URLClassLoader>() {
                    @Override
                    public URLClassLoader run() {
                        return new FilterClassLoader(url, parent);
                    }
                });
        }
        ...
        private static boolean isTarget(String name) {
            for (String prefix : CLASS_FILTER_PREFIX) {
