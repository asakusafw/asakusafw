...
/**
 * Test for {@link StreamRedirectTask}.
 */
public class StreamRedirectTaskTest {
    ...
    public void redirect() {
        StreamRedirectTask t = new StreamRedirectTask(in, out);
        t.run();
    ...
    public void quietExitOnInputError() throws Exception {
        StreamRedirectTask t = new StreamRedirectTask(new ErroneousInputStream(65534), out);
        t.run();
    }
    ...
    public void quietExitOnOutputError() throws Exception {
        StreamRedirectTask t = new StreamRedirectTask(in, new ErroneousOutputStream(65534));
        t.run();
    ...
    public void consumeInputOnOutputError() throws Exception {
        StreamRedirectTask t = new StreamRedirectTask(in, new ErroneousOutputStream(5));
        t.run();
        assertThat("input stream must be consumed", in.read(), is(-1));
    }
    ...
    public void closeInput() {
        StreamRedirectTask t = new StreamRedirectTask(in, out, true, false);
        t.run();
        assertThat(out.toByteArray(), is(BYTES));
    ...
    public void closeOutput() {
        StreamRedirectTask t = new StreamRedirectTask(in, out, false, true);
        t.run();
        assertThat(out.toByteArray(), is(BYTES));
        assertThat(in.closed, is(false));
