...
/**
 * Processes <code>&#64;windgate.csv.line_number</code> attributes.
<h2>'&#64;windgate.csv.line_number' attribute</h2>
The attributed declaration must have no attributes.
 * @since 0.2.4
 */
public class CsvLineNumberDriver  extends PropertyAttributeDriver {

    /**
     * The attribute name.
     */
    public static final String TARGET_NAME = "windgate.csv.line_number"; //$NON-NLS-1$
    ...
    public void process(DmdlSemantics environment, PropertyDeclaration declaration, AstAttribute attribute) {
        ...
        if (CsvFieldDriver.checkConflict(environment, declaration, attribute)) {
            declaration.putTrait(
                    CsvFieldTrait.class,
                    new CsvFieldTrait(attribute, Kind.LINE_NUMBER, null));
        }
    }
}
